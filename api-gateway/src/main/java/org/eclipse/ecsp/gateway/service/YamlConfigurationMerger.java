 ...
 *
 * <p>
 * Precedence rules (FR-039 to FR-043):
 ...
 *
 * <p>
 * Use case: Emergency access management without database changes.
 */
@Service
...
public class YamlConfigurationMerger {
    ...
    private final ClientAccessControlYamlProperties yamlProperties;
    ...
    public List<ClientAccessConfig> merge(List<ClientAccessConfig> databaseConfigs) {
        ...
        if (yamlConfigs.isEmpty()) {
            log.debug("No YAML overrides configured");
        ...
        if (!overriddenClientIds.isEmpty()) {
            log.info("Applied YAML overrides for {} clients: {}", 
                    overriddenClientIds.size(), overriddenClientIds);
        }
        if (!yamlConfigMap.isEmpty()) {
            log.info("Added {} YAML-only clients: {}", 
    ...
    private List<ClientAccessConfig> loadYamlConfigurations() {
        ...
        for (ClientAccessControlYamlProperties.YamlOverride override : yamlProperties.getOverrides()) {
            if (override.getClientId() == null || override.getClientId().isBlank()) {
                log.warn("Skipping YAML override with missing clientId");
                continue;
            }
        ...
        log.debug("Loaded {} YAML configurations", configs.size());
        return configs;
    }
