...
**Client Access Control** provides JWT-based authorization at the API Gateway level, enabling fine-grained access control for OAuth2 client applications accessing backend services.

**Key Capabilities:**
- 🔐 **JWT-Based Client Identification**: Configurable claim name extraction (clientId, azp, client_id, cid)
- 🎯 **Route-Level Access Control**: Service:Route pattern matching with wildcard support
- 🚫 **Deny-by-Default Security**: Explicit allow rules with deny rule precedence
- ⚡ **Real-Time Updates**: Redis Pub/Sub for instant configuration changes
- 🔧 **YAML Override Support**: Environment-specific configurations
- 🛡️ **Security Validation**: SQL injection, XSS, and path traversal detection
- 📊 **Comprehensive Metrics**: Micrometer integration for monitoring
- 📝 **Audit Logging**: All access decisions logged

**Documentation**: See [api-gateway/README.md](api-gateway/README.md) for complete feature documentation including:
...
**Management API**: Client configurations can be managed via the [API Registry](api-registry/README.md) REST endpoints.
...
> **📖 For detailed API documentation including request/response examples, OpenAPI/Swagger access, and testing guides, see [API-Registry README](api-registry/README.md).**
  ...
  gateway:
    client-access-control:
      ...
      skip-paths:
        - /actuator/**
        - /swagger-ui/**
      polling-interval: 60s  # Fallback polling if Redis unavailable
```
...
- **Input Validation**: Protects against SQL injection, XSS, and path traversal attacks in client IDs
- **Audit Logging**: Logs all access control decisions with client ID, service, route, and decision
- **Soft Delete**: Deleted configurations retained for audit trail (use `?permanent=true` for hard delete)

### Example Scenarios

**Allow mobile app to access user and order services:**
```json
{
  "clientId": "mobile-app-v1",
  "allow": ["user-service:*", "order-service:*"]  
}
```

**Allow web app to all services except admin routes:**
```json
{
  "clientId": "web-app",
  "allow": ["*:*", "!admin-service:*", "!user-service:/admin/*"]
}
```

**Allow partner integration limited access:**
```json
{
  "clientId": "partner-xyz",
  "allow": ["product-service:/products", "order-service:/orders/status"]
}
```

### Performance

- **Validation**: < 5ms per request (in-memory cache lookup + rule matching)
- **Cache Refresh**: < 1 second via Redis events
- **Polling Fallback**: 60 second interval (configurable)
