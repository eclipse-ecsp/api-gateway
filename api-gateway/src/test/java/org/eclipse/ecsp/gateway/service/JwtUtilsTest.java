...
class JwtUtilsTest {
    ...
    void testExtractClientId_FirstClaimFound() {
        ...
        // Act
        String clientId = JwtUtils.extractClientId(jwt, List.of("clientId", "azp", "client_id"));
    ...
    void testExtractClientId_CaseInsensitiveMatching() {
        ...
        // Act
        String clientId = JwtUtils.extractClientId(jwt, List.of("clientId", "azp"));
    ...
    void testExtractClientId_NullJwt() {
        // Act
        String clientId = JwtUtils.extractClientId(null, List.of("clientId"));
    ...
    void testExtractClientId_EmptyJwt() {
        // Act
        String clientId = JwtUtils.extractClientId("", List.of("clientId"));
    ...
    void testExtractClientId_TrimWhitespace() {
        ...
        // Act
        String clientId = JwtUtils.extractClientId(jwt, List.of("clientId"));
    ...
    void testExtractClientId_MultipleIdpFormats() {
        ...
        assertEquals("azure_client_id", JwtUtils.extractClientId(azureJwt, List.of("clientId", "azp")));

        // Test OAuth2 standard (client_id)
        String oauth2Jwt = createUnsignedJwt(Claims.class, claims -> {
            claims.put("client_id", "oauth2_client");
        });
        assertEquals("oauth2_client", JwtUtils.extractClientId(oauth2Jwt, List.of("clientId", "client_id")));

        // Test alternative format (cid)
        String altJwt = createUnsignedJwt(Claims.class, claims -> {
            claims.put("cid", "alternative_client");
        });
        assertEquals("alternative_client", JwtUtils.extractClientId(altJwt, List.of("clientId", "cid")));
    }
